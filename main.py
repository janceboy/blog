...
from forms import CreatePostForm, RegisterForm, login_form, CommentForm
from functools import wraps
...
def show_post(post_id):
    ...
    get_comments = db.session.execute(db.select(commentTable))
    all_comments = get_comments.scalars().all()
    ...
    if request.method == "POST":
        if current_user.is_authenticated:
            ...
            return redirect(url_for("login"))
    return render_template("post.html", post=requested_post, comment=comments, all_comments=all_comments)
